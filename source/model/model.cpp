...
int8_t* GetTensorData(TfLiteTensor* tensor, tensor_dims_t* dims, tensor_type_t* type, tensor_params_t* params)
{
    ...
    params->zero_point = tensor->params.zero_point;

    switch (tensor->type)
	{
		case kTfLiteFloat32:
			*type = kTensorType_FLOAT32;
			return reinterpret_cast<int8_t*>(tensor->data.f);
		case kTfLiteUInt8:
...
int8_t* MODEL_GetInputTensorData(tensor_dims_t* dims, tensor_type_t* type, tensor_params_t* inputParams)
{
    ...
    return GetTensorData(inputTensor, dims, type, inputParams);
}
...
int8_t* MODEL_GetOutputTensorData(tensor_dims_t* dims, tensor_type_t* type, tensor_params_t* outputParams)
{
    ...
    return GetTensorData(outputTensor, dims, type, outputParams);
}
